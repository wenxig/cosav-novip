...
import {
  ifCategoriesApiResponse,
  ifCategoriesSubApiResponse,
} from "./interface/CategoriesInterface";
import {
  ifSiteAdtemplateResponse,
  ifSiteFeedbackResponse,
  ifSiteSettingResponse,
  ifSiteSwgApiResponse,
} from "./interface/SiteInterface";
import {
  ifVideoApiResponse,
  ifVideoFavoriteApiResponse,
  ifVideoListApiResponse,
  ifVideoRecommendApiResponse,
} from "./interface/VideoInterface";
import {
  ifAlbumApiResponse,
  ifAlbumContentApiResponse,
  ifAlbumFavoriteApiResponse,
  ifAlbumListApiResponse,
} from "./interface/AlbumInterface";
import { ifGameCategoriesApiResponse, ifGameListApiResponse } from "./interface/GameInterface";
import {
  ifAuthActivityResponse,
  ifAuthChangePasswordApiResponse,
  ifAuthCheckExchangeTicketResponse,
  ifAuthFavoriteAlbumApiResponse,
  ifAuthFavoriteVideoApiResponse,
  ifAuthForgetApiResponse,
  ifAuthGetExchangeTicketResponse,
  ifAuthLoginApiResponse,
  ifAuthRegisterApiResponse,
  ifAuthSponsorApiResponse,
  ifAuthUseExchangeTicketResponse,
...
export const sendSiteFeedback = async (
  ...
  formData.append("download_speed", downloadSpeed);

  const res = await cosApiUtil.sendPost("/site/feedback", formData);
  return res;
};

export const getSiteAdtemplate = async (group: string): Promise<ifSiteAdtemplateResponse> => {
  const formData = new FormData();
 ...
 * @returns
 */
export const sendAuthRegister = async (
  account: string,
  password: string,
  password_confirm: string,
  email: string,
  inviteCode: string,
): Promise<ifAuthRegisterApiResponse> => {
  const formData = new FormData();
  ...
  formData.append("channel_code", channelUtil.getChannelCodeSync());
...
export const sendAuthLogin = async (
  account: string,
  password: string,
): Promise<ifAuthLoginApiResponse> => {
...
export const sendAuthChangePassword = async (
  oldPassword: string,
  newPassword: string,
  password_confirm: string,
): Promise<ifAuthChangePasswordApiResponse> => {
  const formData = new FormData();
...
export const getAuthFavoriteVideoLists = async (
  page: number,
  limit: number,
): Promise<ifAuthFavoriteVideoApiResponse> => {
  const res = await cosApiUtil.sendGet(`/auth/subscribe_video_lists?page=${page}&limit=${limit}`);
  return res;
};

export const getAuthFavoriteAlbumLists = async (
  page: number,
  limit: number,
): Promise<ifAuthFavoriteAlbumApiResponse> => {
  const res = await cosApiUtil.sendGet(`/auth/subscribe_album_lists?page=${page}&limit=${limit}`);
  return res;
};

export const getAuthViewVideoLists = async (
  page: number,
  limit: number,
): Promise<ifAuthFavoriteVideoApiResponse> => {
...
export const getAuthViewAlbumLists = async (
  page: number,
  limit: number,
): Promise<ifAuthFavoriteAlbumApiResponse> => {
...
export const getAuthSponsorLists = async (
  page: number,
  limit: number,
): Promise<ifAuthSponsorApiResponse> => {
  const res = await cosApiUtil.sendGet(`/auth/record_lists?page=${page}&limit=${limit}`);
  return res;
};

export const getAuthActivity = async (): Promise<ifAuthActivityResponse> => {
  const res = await cosApiUtil.sendGet(`/auth/activity`);
  return res;
};

export const authUseExchangeTicket = async (
  videoId: string,
): Promise<ifAuthUseExchangeTicketResponse> => {
  const formData = new FormData();
  formData.append("vid", videoId);
  const res = await cosApiUtil.sendPost("/auth/UseExchangeTicket", formData);
  return res;
};

export const checkExchangeTicket = async (
  videoId: string,
): Promise<ifAuthCheckExchangeTicketResponse> => {
  const formData = new FormData();
  formData.append("vid", videoId);
  const res = await cosApiUtil.sendPost("/auth/CheckExchangeTicket", formData);
  return res;
};

export const getExchangeTicket = async (): Promise<ifAuthGetExchangeTicketResponse> => {
  const res = await cosApiUtil.sendGet("/auth/GetExchangeTicket");
  return res;
};
...
export const getVideoCategoriesSub = async (
  ct: string | undefined,
): Promise<ifCategoriesSubApiResponse> => {
...
export const getVideoList = async (
  ...
  page: number,
  limit: number,
): Promise<ifVideoListApiResponse> => {
  let query = queryStr;
  if (page >= 0) {
    query += `&page=${page}`;
  }
...
export const getAlbumList = async (
  ...
  page: number,
  limit: number,
): Promise<ifAlbumListApiResponse> => {
  let query = queryStr;
  if (page > 0) {
    query += `&page=${page}`;
  }
...
export const getAlbumContent = async (
  albumId: string | undefined,
): Promise<ifAlbumContentApiResponse> => {
