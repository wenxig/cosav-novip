...
export class CosApiUtil{
    ...
    private readonly apiCloudServer = process.env.REACT_APP_API_CLOUD_SERVER || "https://di4n3-c01.tos-cn-hongkong.bytepluses.com/cossvr2025.txt"
    private readonly defaultApiUrl = process.env.REACT_APP_DEFAULT_API_URL || "https://api.cos-dkop.net/api/app/v1"
    private readonly disableApiCloud = process.env.REACT_APP_DISABLE_API_CLOUD === "true"
    ...
    private decryptData(data :string) : string{
        try{
            ...
            return dData.toString(cryptoJs.enc.Utf8);
        }catch(e){
            console.log(e)
        }
        return 'decrypt fail'
    ...
    public async getApiUrl():Promise<string>{
        ...
        if(this.disableApiCloud){
            return this.setGetApiResult(this.defaultApiUrl,'api cloud disabled by env')
        }
        ...
        try{
            ...
            if(res.ok){
                ...
                const url = this.decryptData(textData)
    
                if(url === 'decrypt fail' || url === ''){
                    return this.setGetApiResult(this.defaultApiUrl,'decrypt cloud data fail')
                }
    ...
    public sendGet = async (apiName :string,useAuthId:boolean = false):Promise<getResult> => {
        ...
        try{
            ...
            try{
                ...
                if (jsonData.code === 200 || jsonData.code === 201 || jsonData.code === 0) {
                    return {
                        result: 'success' as const,
                        message: null,
                        data: JSON.parse(this.decryptData(jsonData.data))
                    };
                }
        ...
        try {
            ...
            try{
                ...
                try{
                    return {
                        result: 'fail' as const,
                        message: jsonData.errorMsg?jsonData.errorMsg.msg.join(','):`API error code with: ${jsonData.code}`,
                        data: jsonData.data?JSON.parse(this.decryptData(jsonData.data)):null
                    };
                }catch(e){
                    return {
                        result: 'fail' as const,
