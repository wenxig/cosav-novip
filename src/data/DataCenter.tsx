...
export const storedUserInfo = create<UserInfoState>()(
  persist(
    (set) => ({
      ...
      favoriteVideoList: [],
      favoriteAlbumList: [],
      setUserInfo: (info: ifAuthLogin) =>
        set({
          userInfo: info,
          userParams: info.Userparams,
          userAuthId: info.auth_id,
        }),
      setUserParams: (params: string) =>
        set({
          userParams: params,
        }),
      ...
      setFavoriteVideoList: (list: string[]) =>
        set({
          favoriteVideoList: list,
        }),
      setFavoriteAlbumList: (list: string[]) =>
        set({
...
export const setUserInfo = (userInfo: ifAuthLogin) => {
  ...
  getAuthFavoriteVideoLists(0, 9999).then((res) => {
    if (res.result === "success" && res.data) {
      storedUserInfo
        .getState()
        .setFavoriteVideoList(res.data.lists.map((item: ifVideoBaseInfo) => item.id));
    }
  });
  ...
  getAuthFavoriteAlbumLists(0, 9999).then((res) => {
    if (res.result === "success" && res.data) {
      storedUserInfo
        .getState()
        .setFavoriteAlbumList(res.data.lists.map((item: ifAlbumBaseInfo) => item.id));
    }
  });
};
...
export const addFavoriteVideo = (videoId: string) => {
  storedUserInfo
    .getState()
    .setFavoriteVideoList([...storedUserInfo.getState().favoriteVideoList, videoId]);
};
...
export const addFavoriteAlbum = (albumId: string) => {
  storedUserInfo
    .getState()
    .setFavoriteAlbumList([...storedUserInfo.getState().favoriteAlbumList, albumId]);
};
...
export const storedSiteData = create<SiteDataState>()(
  persist(
    (set) => ({
      ...
      setSiteSetting: (info: ifSiteSetting) =>
        set({
          siteSetting: info,
        }),
      setCategories: (categories: ifCategoryItem[]) =>
        set({
...
export const storedGameData = create<GameDataState>()(
  persist(
    (set) => ({
      ...
      setGameCategories: (categories: ifGameCategories[]) =>
        set({
          gameCategories: categories,
        }),
      setGameTop10: (top10: ifGameList) =>
        set({
          gameTop10: top10,
        }),
    }),
...
export const usePlaybackSpeed = create<PlaybackSpeedState>()((set, get) => ({
  ...
  m3u8LoadCount: 0,
  _bufferingStart: 0,
  addSpeedSample: (kbps: number, durationMs?: number) =>
    set((state) => {
...
export const storedTempData = create<TempDataState>()(
  persist(
    (set) => ({
      ...
      setFeedbackVideoData: (data: TempFeedbacVideoData) =>
        set({
