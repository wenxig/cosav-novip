...
function CosRegistPage() {
  ...
  const handleRegister2 = async () => {
    ...
    try {
      ...
      //API回傳失敗訊息
      if (res.data && res.data.status.includes("fail")) {
        setShowMsg(res.data.msg.join(","), "error");
        return;
      }
      ...
      // 上报注册事件（fire & forget，不阻塞流程）
      trackerUtil.track("register").catch(() => {});
                  ...
                  control={
                    <Checkbox
                      ...
                      sx={{
                        color: "white", // 未勾選時的顏色
                        "&.Mui-checked": {
                          color: cMainColor3, // 勾選後的顏色
                        },
                      }}
                    />
                  }
                  label="服务条款"
                  sx={{ alignSelf: "flex-start", color: "white", ml: 3 }}
                />
